...
lines.push('')
lines.push(`maratool.com is a growing collection of ${live.length} free tools that run entirely in the browser using vanilla JavaScript and the Web APIs. Every tool is statically pre-rendered and served from Cloudflare's edge network. No data is sent to any server. No sign-up required for any tool.`)
lines.push('')
lines.push('## Tools')
lines.push('')
