...
Add an **Exercise Database** to maratool.com: **1,035 exercise pages**, ~41
...
**Merges: 99.** 6 records are dropped rather than defaulted (1 Everkinetic record with no
primary muscle, 5 free-exercise-db records with no instructions in the raw upstream data —
see §3's merge rule). Merged total: **1,035 unique exercises** — 266 with vector animation,
769 photo-only.
...
Jaccard ≥ 0.75). Every (Everkinetic, free-db) candidate pair scoring at or
above the threshold is collected up front — across the whole dataset, not
one Everkinetic record at a time — then assigned **greedily from the highest
score down** (ties broken by Everkinetic `id_num`, then free-db `id`, for
reproducibility), so a mediocre early match can never consume a row a better,
later candidate needed; each free-db record is consumable by at most one
match. A **distinguishing-modifier guard** rejects a candidate pair outright,
independent of score, when the two names disagree on equipment (`band` vs
`barbell` vs `dumbbell` vs `cable` vs `machine` vs `kettlebell` vs `smith` —
conflict only when *both* sides name equipment and it differs) or on bench
angle (`incline`/`decline`/`flat` — conflict even when only *one* side names
an angle, since angle changes which muscle heads are worked and silence is
not a safe wildcard). Stance (`seated`/`standing`/`lying`/`kneeling`) uses the
same lenient rule as equipment, since it changes stability/comfort rather
than the movement pattern. On a match, keep **one** record and prefer the
**Everkinetic vector media** (better UX), merging free-db's richer metadata
(`level`, `force`, `mechanic`, `category`). Non-matches from both sides pass
through. **A record is dropped (and logged by name) rather than defaulted**
if, after merging, it still has no primary muscle or no instructions — never
invent anatomy or fabricate steps; a free-db record with fewer than 2 images
is dropped the same way. The match threshold, the modifier guard, and the
resulting merge map (as `"<Everkinetic name>" => "<free-db name>"` pairs, not
just a count) are **snapshot-tested** so the merge can't silently drift.
...
group gets `{slug}-{n}`, where `n` is the member's 1-based position within the
colliding group (stable input order, not a source id). Deterministic;
...
|---|---|---|
| Browser | `/exercises` | 1 |
| Individual exercise | `/exercises/{slug}` | 1,035 |
...
**≈1,077 pages.** Hubs are namespaced (`/muscle/`, `/equipment/`, …) so
...
- **Registry:** one `tools.ts` entry for the browser (slug `exercises`,
  ...
  ~1,077 generated pages are routes, not registry entries.
...
- Merge fidelity: output count === 1,035; every source record either present,
...
- **Coverage asymmetry** — only 266/1,035 get vector animation; the other 769
...
- [ ] `npm run build` clean; ~1,077 exercise routes emitted.
